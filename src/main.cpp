...
int main(int argc, char** argv) {
    ...
    std::cout<<"HERE WE GO2!";

    //open RTSP
    if (avformat_open_input(&format_ctx, "rtsp://admin:admin123@192.168.0.90/axis-media/media.3gp",
            NULL, NULL) != 0) {
        return EXIT_FAILURE;
    }
    ...
    while (av_read_frame(format_ctx, &packet) >= 0 && cnt < 500) { //read ~ 1000 frames
